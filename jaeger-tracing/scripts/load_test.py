...
class LoadTester:
    ...
    async def _make_request(
        self, client: httpx.AsyncClient, user_id: int, request_id: int
    ) -> None:
        ...
        try:
            if request_type == "health":
                ...
                response = await client.get(
                    f"{self.base_url}/demo/full-flow/{user_id_param}"
                )
                result = RequestResult(
    ...
    def _calculate_metrics(self, total_time: float) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "error_rate": len([r for r in self.results if r.status_code >= 400])
            / len(self.results)
            * 100,
...
async def main():
    ...
    print("FastAPI Load Testing Tool")
    ...
    print("   Look for performance traces and bottlenecks")
