...
class TrafficGenerator:
    ...
    def __init__(
        self, base_url: str = "http://localhost:8000", concurrent_requests: int = 5
    ):
        self.base_url = base_url.rstrip("/")
        self.concurrent_requests = concurrent_requests
        self.session_timeout = httpx.Timeout(30.0)

    async def generate_user_activity(
        self, client: httpx.AsyncClient, duration: int = 60
    ) -> None:
        ...
        print(
            f"Completed user activity generation. Total patterns executed: {request_count}"
        )
        ...
        try:
            ...
            if order_response.status_code == 200:
                ...
                payment_response = await client.post(
                    f"{self.base_url}/payments", json=payment_data
                )
                if payment_response.status_code == 200:
                    payment = payment_response.json()
                    print(
                        f"Completed full flow: User {user_id} -> Order {order['id']} -> Payment {payment['id']}"
                    )
    ...
    async def _error_scenario_pattern(self, client: httpx.AsyncClient) -> None:
        ...
        scenarios = [
            ...
            (
                "invalid_order_data",
                lambda: client.post(
                    f"{self.base_url}/orders", json={"invalid": "data"}
                ),
            ),
            (
                "invalid_payment_data",
                lambda: client.post(
                    f"{self.base_url}/payments", json={"invalid": "data"}
                ),
            ),
        ]
    ...
    async def demo_full_flow(self, client: httpx.AsyncClient) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Order ID: {result['order']['id']}, Payment Status: {result['payment']['status']}"
                )
            else:
    ...
    async def stress_test(
        ...
        print(
            f"Starting stress test: {requests_per_second} RPS for {duration} seconds..."
        )
...
async def main():
    parser = argparse.ArgumentParser(
        description="FastAPI Jaeger Tracing Traffic Generator"
    )
    parser.add_argument(
        "--url",
    ...
    parser.add_argument(
        "--rps", type=int, default=10, help="Requests per second (stress mode only)"
    )
    ...
    print("FastAPI Jaeger Tracing Traffic Generator")
    ...
    print("   Look for traces from service: fastapi-jaeger-demo")
