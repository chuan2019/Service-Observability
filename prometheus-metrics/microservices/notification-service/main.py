...
from shared.schemas import NotificationCreate, NotificationResponse, HealthResponse, NotificationStatus
    ...
    with NOTIFICATION_OPERATION_DURATION.labels(operation="send").time():
        try:
            ...
            if success:
                ...
                NOTIFICATION_OPERATIONS.labels(
                    operation="send",
                    type=notification_data.type,
                    status="success"
                ).inc()
...
async def get_order_notifications(
    ...
    with NOTIFICATION_OPERATION_DURATION.labels(operation="get_order_notifications").time():
        try:
            ...
            NOTIFICATION_OPERATIONS.labels(
                operation="get_order_notifications",
                type="order",
                status="success"
            ).inc()
            return [NotificationResponse.model_validate(notif) for notif in notifications]
