...
from shared.schemas import HealthResponse, ProductCreate, ProductResponse, ProductUpdate
...
TOTAL_PRODUCTS = Gauge("product_service_total_products", "Total number of products", registry=REGISTRY)
...
app.add_middleware(PrometheusMiddleware, service_name="product-service")
...
async def create_product(product_data: ProductCreate, session: AsyncSession = Depends(get_session)):
    ...
    with PRODUCT_OPERATION_DURATION.labels(operation="create").time():
        try:
            ...
            existing_product = await session.execute(select(Product).where(Product.sku == product_data.sku))
...
async def update_product(product_id: int, product_data: ProductUpdate, session: AsyncSession = Depends(get_session)):
    """Update a product."""
