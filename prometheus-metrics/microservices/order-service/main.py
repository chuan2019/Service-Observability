...
ACTIVE_ORDERS = Gauge("order_service_active_orders", "Number of active orders", registry=REGISTRY)

ORDER_VALUE = Histogram("order_service_value", "Order values", ["status"], registry=REGISTRY)
...
async def create_order(order_data: OrderCreate, session: AsyncSession = Depends(get_session)):
    ...
    with ORDER_OPERATION_DURATION.labels(operation="create").time():
        try:
            ...
            # Create order
            order = Order(user_id=order_data.user_id, status=OrderStatus.PENDING, total_amount=total_amount)
            session.add(order)
            await session.flush()
...
async def list_orders(skip: int = 0, limit: int = 100, session: AsyncSession = Depends(get_session)):
    """Get all orders."""
...
async def cancel_order(order_id: int, reason: str = "user_requested", session: AsyncSession = Depends(get_session)):
    """Cancel an order."""
